...
sys.stdout = open('./code/adapted_deep_embeddings/Tiny_Tf.txt','wt')
...
def main():
    ...
    (train1_x, train1_y), (test1_x, test1_y), (train2_x, train2_y), (test2_x, test2_y)  = dataset.kntl_data_form(k, n, 300, n)
    # images = list(zip(X, Y))
    # random.shuffle(images)
    # img_train, img_test = train_test_split(images, test_size=0.2)
    ...
    train1_y=to_categorical(train1_y)
    test1_y=to_categorical(test1_y)
    ...
    image_gen.fit(train1_x)

    img_gen=image_gen.flow(train1_x, train1_y, batch_size=BS, shuffle=True)
    ...
    train_model = model50.fit_generator(img_gen, validation_data=(test1_x, test1_y), epochs=epoch, steps_per_epoch=len(train1_x)//BS, verbose=1)
    ...
    image_gen2 = ImageDataGenerator(
                               data_format='channels_last')
    img_gen2=image_gen2.flow(train2_x, train2_y, batch_size=BS, shuffle=True)
    train_model = model50.fit_generator(img_gen2, validation_data=(test2_x, test2_y), epochs=epoch, steps_per_epoch=len(train2_x)//BS, verbose=1, callbacks=[tensorboard])

    (loss, accuracy) =  model50.evaluate(test2_x, test2_y, batch_size=10, verbose=1)
    print( 'loss = {:.4f}, accuracy: {:.4f}%'.format(loss,accuracy*100))


    test_pred = model50.predict(test2_x, verbose=1, batch_size=64).argmax(axis=1)
    test_true=test2_y.argmax(axis=1) 
